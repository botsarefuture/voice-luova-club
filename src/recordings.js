...
export async function encryptRecording(blob, key) {
...
export async function decryptRecording(ciphertext, iv, mimeType, key) {
  const plaintext = await crypto.subtle.decrypt({ name: "AES-GCM", iv: new Uint8Array(iv) }, key, await ciphertext.arrayBuffer());
