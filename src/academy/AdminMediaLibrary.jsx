...
function MediaField({ label, value, onChange, multiline = false, required = false, ...props }) {
  const controls = { value: value ?? "", onChange: (event) => onChange(event.target.value), required, ...props };
  return <label className={multiline ? "admin-field admin-field-wide" : "admin-field"}>{label}{multiline ? <textarea {...controls} rows="4" /> : <input {...controls} />}</label>;
}
