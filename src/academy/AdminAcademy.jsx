...
export default function AdminAcademy({ roles }) {
  ...
  function startBlankLesson() {
    const draft = createBlankLesson();
    setLessonDraft(draft, "New lesson created locally. Give it a clear title and stable ID before saving.");
    setChangeNote("Created a new lesson.");
    resetLessonWorkflow();
  }

  function startNextLessonRevision() {
    if (!lesson) return;
    const draft = createNextLessonRevision(lesson);
    setLessonDraft(draft, `Version ${draft.version} created as a private draft. The published revision is unchanged.`);
    setChangeNote(`Started from published version ${lesson.version}.`);
    resetLessonWorkflow();
  }

  function resetLessonWorkflow() {
    setLessonStatus("draft");
    setReview({ decision: "approved", content_checked: false, research_checked: false, accessibility_checked: false, note: "" });
    setComparison(null);
    setPreview(false);
  ...
  function startBlankCourse() {
    setCourseDraft(createBlankCourse(), "New course created locally. Add lessons when their drafts are ready.");
    resetCourseWorkflow();
  }

  function startNextCourseRevision() {
    if (!course) return;
    const draft = createNextCourseRevision(course);
    setCourseDraft(draft, `Course version ${draft.version} created as a private draft.`);
    resetCourseWorkflow();
  }

  function resetCourseWorkflow() {
    setCourseStatus("draft");
    setCourseReview({ decision: "approved", content_checked: false, research_checked: false, accessibility_checked: false, note: "" });
  }
  ...
  async function saveCourse() {
    ...
    try {
      ...
      await saveAcademyAdminCourse(course.id, course.version ?? 1, course);
      setCourseStatus("draft");
  ...
  async function courseTransition(action) {
    if (!course) return;
    try {
      const version = course.version ?? 1;
      const result = action === "submit" ? await submitAcademyAdminCourseForReview(course.id, version) : action === "review" ? await reviewAcademyAdminCourse(course.id, version, courseReview) : await publishAcademyAdminCourse(course.id, version);
  ...
  return <section className="admin-academy-page" aria-labelledby="admin-academy-title">
    ...
    <div className="admin-academy-grid">
      <aside aria-label="Saved Academy lessons"><div className="admin-academy-list-heading"><h3>Lessons</h3><button type="button" className="secondary-action" onClick={startBlankLesson}><FilePlus2 /> New lesson</button></div><label className="admin-reference-select">Start from Foundations<select value="" onChange={(event) => event.target.value && startFoundationsReference(Number(event.target.value))}><option value="">Choose a reference...</option>{FOUNDATIONS_LESSONS.map((reference, index) => <option value={index} key={reference.id}>{reference.title}</option>)}</select></label>{records.length ? <ol>{records.map((record) => <li key={`${record.lesson_id}:${record.version}`}><button type="button" onClick={() => openRecord(record)}><strong>{record.title}</strong><span>v{record.version} · {record.status}</span></button></li>)}</ol> : <p className="lesson-muted">No saved lesson revisions yet.</p>}</aside>
      <div className="admin-academy-editor">
        {!lesson ? <EmptyAuthoringState onCreate={startBlankLesson} onReference={() => startFoundationsReference()} /> : <>
          ...
          <PublishingWorkflow lesson={lesson} lessonStatus={lessonStatus} review={review} onReviewChange={setReview} roles={roles} onSubmit={submitForReview} onReview={completeReview} onPublish={publishRevision} onNewRevision={startNextLessonRevision} />
...
function EmptyAuthoringState({ onCreate, onReference }) {
  return <section className="admin-empty-authoring"><h3>Create a lesson</h3><p>Begin with a clean lesson or use reviewed Foundations content as a reference. Nothing is published until review and publishing are complete.</p><div className="admin-academy-actions"><button type="button" className="primary-action" onClick={onCreate}><FilePlus2 /> New blank lesson</button><button type="button" className="secondary-action" onClick={onReference}>Load Foundations welcome</button></div></section>;
}
...
function PublishingWorkflow({ lesson, lessonStatus, review, onReviewChange, roles, onSubmit, onReview, onPublish, onNewRevision }) {
...
function CourseEditor({ course, records, courses, onCreate, onStart, onOpen, onChange, onSave, onNewRevision, canAuthor, status, review, onReviewChange, roles, onTransition }) {
