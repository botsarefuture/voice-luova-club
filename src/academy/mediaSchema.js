...
export function validateMediaAsset(asset, { requireReview = false, publicationReady = false } = {}) {
  ...
  if (text(asset.source) && !safeDeliverySource(asset.source)) errors.push("Source must be a same-origin path.");
  ...
  if (text(asset.rights?.sourceUrl) && !safeRightsSource(asset.rights.sourceUrl)) errors.push("Original source must be an HTTPS URL.");
  ...
  if (publicationReady && asset.kind === "video" && !safeDeliverySource(asset.accessibility?.captions)) errors.push("Video needs a same-origin captions path before review.");
...
function safeDeliverySource(value) {
  if (!text(value)) return false;
  return value.startsWith("/") && !value.startsWith("//");
}

function safeRightsSource(value) {
